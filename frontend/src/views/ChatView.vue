    ...
    <section class="chat-panel">
      ...
      <div ref="scrollArea" class="conversation" @scroll="trackScroll">
        ...
        <article v-for="(turn, index) in turns" :key="turn.id || index" class="turn">
          ...
          <AnswerContent v-if="turn.answer" :text="turn.answer" :citations="turn.citations || []" :pending="turn.pending" @source="openEvidence" />
          ...
          <details v-if="turn.citations?.length" class="sources"><summary>引用来源 · {{ turn.citations.length }}</summary><div v-for="(cite, ci) in turn.citations" :key="ci" class="source"><button type="button" class="evidence-title-button" @click="openEvidence(cite)"><b>{{ Number(ci) + 1 }}. {{ cite.title }}</b></button><small>{{ cite.document_title }} {{ cite.chapter }} {{ cite.section }} <template v-if="cite.page">· 第 {{ cite.page }} 页</template></small><small v-if="cite.doi">DOI：{{ cite.doi }}</small><button v-if="isPdf(cite)" type="button" class="source-link" @click="openEvidence(cite)">查看原 PDF ↗</button></div></details>
          ...
          <div v-if="!turn.pending" class="turn-actions"><a-button size="small" type="text" @click="copyAnswer(turn)">复制回答</a-button><a-button v-if="turn.error" size="small" type="text" :disabled="busy" @click="draft = turn.query">重新编辑问题</a-button><small v-if="turn.elapsed_ms">{{ (turn.elapsed_ms / 1000).toFixed(1) }} 秒</small></div>
        </article>
      </div>
    ...
    <a-modal v-model:open="showSource" :title="source?.title" :width="1200" :footer="null" destroy-on-close>
      <div v-if="source" class="source-modal">
        <div class="source-text">
          ...
          <h4>{{ source.kind === 'figure' ? '架构/电路图' : source.kind === 'equation' ? '公式' : '知识点摘录' }}</h4>
          <div class="source-content" v-html="renderAnswer(equationBody(source) || source.content || '')"></div>
          <DocumentAsset v-if="source.image_key || source.image_url" :document-id="source.document_id || ''" :image-key="source.image_key" :image-url="source.image_key ? undefined : source.image_url" :alt="source.title || '知识库图片'" />
        </div>
        <PdfSourceViewer v-if="isPdf(source) && !textOnly" :document-id="source.document_id" :filename="source.filename" :source-page="source.page" :source-key="source.knowledge_id || source.id" @show-text="textOnly = true" />
      </div>
    </a-modal>
  </div>
</template>
...
const showSource = ref(false), source = ref<any>(), textOnly = ref(false);
...
function equationBody(cite: any) {
  const latex = String(cite?.latex || '').trim();
  if (latex) return `$$\n${latex}\n$$`;
  return cite?.content || '';
}
function isPdf(item: any) {
  const name = String(item?.filename || '');
  return Boolean(item?.document_id && (!name || /\.pdf$/i.test(name)));
}
function openEvidence(item: any) { source.value = item; textOnly.value = false; showSource.value = true; }
...
async function copyAnswer(turn: any) {
  const text = (turn.answer || '').replace(/\[\[figure:([^\]]+)\]\]/g, (_: string, id: string) => {
    const cite = (turn.citations || []).find((item: any) => item.knowledge_id === id);
    return cite ? `【图：${cite.title || '知识库图片'}${cite.page ? `，第 ${cite.page} 页` : ''}】` : '【图片暂不可用】';
  });
  try { await navigator.clipboard.writeText(text || ''); message.success('已复制'); } catch { message.error('复制失败，请手动选择文字复制'); } }
